...
public class Camera {
	...
	public Ray constructRay(int nX, int nY, int j, int i)
	{
		...
		{
			Point pIJ = middlePixel(nX, nY ,j,i);
	...
	public Camera renderImage() {
		 ...
		 pixelManager = new PixelManager(nY, nX, 5);
		 if (threadsCount == 0) {
			 for (int i = 0; i < nY; ++i)
			   for (int j = 0; j < nX; ++j)
			       castRay(nX, nY, j, i);
...
//		try {
		...
		for(int i=0 ; i < im.getNy() ; i++)
			...
			{
				Color color = castRayOld(im.getNx(), im.getNy(),j ,i);
...
//		}
     ...
     * @return the color
     */
    private Color castRayOld(int nX, int nY, int j, int i) {
        if (useAdaptive == true)
        {
        	Point p = middlePixel(nX, nY, j, i);      
            return adaptiveHelper(p, nX, nY);
        }
        else
        {
            return rtb.traceMultiRay(constructMultiRay(nX, nY, j, i));
        }
    }
